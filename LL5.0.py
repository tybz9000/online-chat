...
def contract(event):#主连接
    global k
    data= et4.get()
    if len(et4.get()) == 0 or data=='请输入内容':
        et4.delete(0,tkinter.END)
        et4.insert(tkinter.END,'请输入内容')#防止用户什么都不输刷屏
    else:
    ...
    def cout(i):#递归防止程序挂起
        ...
        if(i>0):
            try:
                ...
                if datad=='ok':#如果信息是ok
                    os.startfile('file1c.pyw')#打开文件传输模块
                elif datad=="sbwanttosendyouafile":#或者秘钥是我想发文件
                    hostt2=open('cr.txt','w')#以写的形式打开cr.txt文件，用以传递参数
                    hostt2.write(str(host)+' '+str(dk+1))#写入ip和端口
                    hostt2.close()#关闭文件
                    ...
                    for m in range(0, len(data)):#对于每一个表情
                        if str(data[m]).isdigit() is True:#如果它是数字
                            ...
                            p+=1#计数
                            imt2.append(ImageTk.PhotoImage(imt))
                            t.image_create(tkinter.END,image = imt2[p])#插入表情
                        else:#如果它不是表情
...
def file_send(event):#定义文件发送函数
    ...
    hostt=open('c.txt','w')#打开文件，用以传递参数
    hostt.write(str(host)+' '+str(dk+1))#写入ip和端口
    hostt.close()#关闭文件 
    root2=tkinter.Tk()#弹出另外一个对话框
    root2.title("准备发送")#题目
    root2['background']='lightblue'#背景色
    root2.geometry('180x60+150+150')#大小
    address=str(t2.curselection())[1]#地址
    def confirm(event):#确认对话框
        global address,host,dk,data#全局变量，防止未定义情况出现
        data='Iwanttosendsomething'#秘钥
        sock.sendto(data.encode(),(host,dk))#发送给服务端
        sock.sendto(address.encode(),(host,dk))#再发送
        root2.destroy()#关闭此窗口
    def destroy(event):#定义关闭函数
        time.sleep(0.4)
        root2.destroy()
    lb1=ttk.Label(root2,text='确认向对方传输文件？')#lable 提示语
    lb1.place(x=30,y=0)
