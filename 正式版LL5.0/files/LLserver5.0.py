...
serverdk.bind((socket.gethostbyname(socket.gethostname()),dk))#绑定ip值######！！！！！！！！
...
while True:#挂起循环
    ...
    if data=='Iwanttosendsomething':#秘钥识别
        ...
        hostt.write(str(ip)+' '+str(dk2))#写入ip和地址
        hostt.close()#关闭程序
        os.startfile('file1s.pyw')#打开tcp传输服务器
        serverdk.sendto('ok'.encode(),addr)#发送可以传的密令
        serverdk.sendto(str(dk2).encode(),addr)
        time.sleep(0.1)
        serverdk.sendto("sbwanttosendyouafile".encode(),addrl[data])#识别密令
        serverdk.sendto(str(dk2).encode(),addrl[data])#解密
    elif(data=='iwanttochangemyname'):
        data1,addr=serverdk.recvfrom(4096)
        data1=data1.decode()
        x=0
        while(x<=a):
            if(addr==addrl[x]):
